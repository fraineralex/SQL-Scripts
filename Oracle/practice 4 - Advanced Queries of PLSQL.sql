/* 7.1 Construya un c�digo PL/SQL que lea el ID del empleado, ID del supervisor y comisi�n. 
   Si el empleado cobra comisi�n que guarde en una variable "Cobra comisi�n". Adem�s, si el 
   ...
   <<Mensaje>>".Guarde la informaci�n le�da de la tabla de empleados en una variable 
   tipo RECORD. ermitir que el c�digo sea reusable */
...
departamento y el nivel salarial (para simplificar la construcci�n de la variable RECORD cree
una vista). En el cuerpo del c�digo, deber�n leer la informaci�n correspondiente de las tablas
de empleados, departamentos y niveles salariales y guardar los datos en la variable tipo
RECORD. A continuaci�n deber�n construir una estructura CASE para evaluar el nivel salarial.
Deber�n almacenar en una variable un texto dependiendo del nivel salarial. Si es "A"
almacenan "Low", si es "B" o "C" almacenan "Medium", si es "D" almacenan "High", de lo
contrario almacenan "Elite". Finalmente deber�n insertar en la tabla creada inicialmente los
valores obtenidos. Fuera del c�digo, revise el contenido de la tabla EMP_GRADES.*/

-- Creaci�n de la tabla EMP_GRADES
...
/*  7.3 Construir un c�digo PL/SQL usando una estructura de arreglo asociativa (INDEX-BY table).
En la misma deber�s guardar los registros de la tabla de empleados para los ID de empleados del
100 al 120. Las siguientes tareas deber�n realizarse a partir del arreglo asociativo construido
y usando los m�todos de tablas INDEX BY:

  1. Mostrar en pantalla la cantidad de registros.
  2. Mostrar el apellido del primer registro.
  3. Mostrar el apellido del �ltimo registro.
 ...
 de que un �ndice no exista en dicho rango, mostrar "No existe".
...
BEGIN 
 ...
 -- 3. Mostrar el apellido del �ltimo registro.   
   DBMS_OUTPUT.PUT_LINE('3. Mostrar el apellido del �ltimo registro.'|| CHR(10) ||
   '- Apellido del �ltimo registro: ' || emp_array(emp_array.LAST).last_name || CHR(10));
 ...
 -- de que un �ndice no exista en dicho rango, mostrar "No existe".
    ...
    en caso de que un �ndice no exista en dicho rango, mostrar "No existe' || CHR(10));
    FOR i IN 100..120 LOOP
...
/*7.4 Construir un c�digo PL/SQL que cree una variable tipo VARRAY de tama�o 21 
...
El tama�o del arreglo debe ser de 25. Luego deber�n crear una variable 
...
por pantalla. Mostrar la informaci�n de la siguiente forma:

           Departamento: <<ID del departamento>>
                *** Empleado: <<Apellido>>
Usen el comando para definir una variable de sustituci�n y para
...
/* 8.1 Construya un c�digo PL/SQL que usando un CURSOR expl�cito busque la fecha m�xima
de contrataci�n en la tabla de empleados. Use FETCH para leer cada fila del cursor.
Analice cu�ntos registros retornar� el cursor. Basado en su an�lisis elija la mejor
forma de construir el c�digo. Muestre el siguiente mensaje:
                      La m�xima fecha es <<Fecha>>            */
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('La m�xima Fecha es: ' || v_fecha);
...
/* 8.2 Construya un c�digo PL/SQL usando un CURSOR expl�cito que busqueel apellido
y la posici�n de todos los empleados que se reportan al supervisor 100.
Mostrar el siguiente mensaje s�lo para los primeros 10 empleados:

          El empleado <<Apellido>> trabaja como <<Posici�n>>
...
/* 8.3 Construya un c�digo PL/SQL usando cursor FOR LOOP que lea los ID del empleado,apellido, 
...
salarial. Mueste la informaci�n por pantalla como sigue:
...
BEGIN
     DBMS_OUTPUT.PUT_LINE('INFORMACI�N DE LOS EMPLEADOS' || CHR(10));
...
/* 8.4 Construya un c�digo PL/SQL usando un cursor FOR LOOP con subquery. Muestre el ID,
apellido y pa�s de todos los empleados que trabajen en Canad� y United Kingdom. Ordene
la data por pa�s y apellido de manera ascendente. El formato de la salida debe ser como sigue
                       ID *** Apellido *** Pa�s                   */
SET SERVEROUTPUT ON                   
BEGIN
    DBMS_OUTPUT.PUT_LINE('INFORMACI�N DE LOS EMPLEADOS' || CHR(10));
    ...
    LOOP
       DBMS_OUTPUT.PUT_LINE('ID: ' || v_emp.employee_id || ' *** ' || 'Apellido: ' ||
                            v_emp.last_name || ' *** ' || 'Pa�s: ' || v_emp.country_name);
...
/* 8.5 Construya un c�digo PL/SQL con un cursor que busque el salario total y el salario
promedio que trabajan en un departamento dado. Al momento de abrir el cursor pase los 
valores 20 y 30 como par�metros. Muestre la informaci�n como sigue:
...
/* 8.6 Construya un c�digo PL/SQL que busque el ID y salario de los empleados que ganan una comisi�n
mayor a un valor dado (Ejemplo: 0.1) y bloquee el salario para que no sea actualizado por otra sesi�n.
En la secci�n de ejecuci�n actualice el salario de los empleados que vengan en el cursor en un 10%.
Para realizar esta pr�ctica use un cursor FOR LOOP con par�metros, FOR UDPATE OF, NOWAIT y CURRENT OF.*/
...
/* 8.7 Realice la pr�ctica 8.6 usando WAIT por 10 segundos. Conectado como SYSTEM cree un usuario
(otorgarles los privilegios correspondientes para que pueda crear una sesi�n), luego conectado
como HR se le debe otorgar los privilegios de actualizaci�n sobre la tabla EMPLOYEES. Cree una
sesi�n nueva conectado con el usuario recientemente creado y actualice el salario en un 15% al
empleado 146 (no asiente los cambios). En la sesi�n HR, corra el programa y en un primer 
escenario deje que pase los 10 segundos y comente qu� pasa. En la misma sesi�n de HR, c
orra nuevamente el programa y antes de que venzan los 10 segundos, dir�jase a la sesi�n 
